...
class Coordinator:
    ...
    async def register_agent(
        self, agent_id: str, capabilities: List[str], agent_instance: Any = None
    ) -> None:
    ...
    async def submit_task(self, task_type: str, input_data: Dict[str, Any]) -> str:
        ...
        # Create task
        task = Task(task_type=task_type, agent_id=selected_agent, input_data=input_data)
        ...
        for agent in agents:
            if hasattr(agent, "hrm") and agent.hrm is not None:
                ...
                if hrm_status["goals_count"] > 0:
                    ...
                    coordination_results["total_actions"] += len(
                        reasoning_output.get("actions", [])
                    )
                    ...
                    self.logger.info(
                        f"Coordinated HRM for agent {agent.agent_id}: {len(reasoning_output.get('actions', []))} actions generated"
                    )
                else:
