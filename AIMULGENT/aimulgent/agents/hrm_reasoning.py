...
class AdaptiveComputationTime(nn.Module):
    ...
    def __init__(
        self, hidden_size: int, max_steps: int = 10, exploration_prob: float = 0.1
    ):
        super().__init__()
    ...
    def forward(
        self, hidden_state: torch.Tensor, step: int, training: bool = False
        ...
        if training and torch.rand(1).item() < self.exploration_prob:
            ...
            min_steps = max(
                2, min(self.max_steps, step + torch.randint(1, 4, (1,)).item())
            )
        ...
        return should_halt.squeeze(), (
            q_values.squeeze(0) if q_values.shape[0] == 1 else q_values
        )
...
class HierarchicalReasoningModel(nn.Module):
    ...
    def forward(
        self, input_embedding: torch.Tensor, steps: int = 0
        ...
        if self.input_proj is not None:
            ...
            if not hasattr(self, "_temp_input_proj"):
                self._temp_input_proj = nn.Linear(
                    input_embedding.size(-1), self.hidden_size, bias=False
        ...
        if self.use_iterative_refinement:
            return self._forward_with_refinement(
                h_state, l_state, input_embedding, steps
            )
        else:
        ...
        for h_cycle in range(self.h_cycles):
            ...
            for l_cycle in range(self.l_cycles):
                for l_layer in self.l_level:
                    ...
                    combined_input = (
                        l_state + input_embedding + h_state * 0.1
                    )  # Small H influence
                    ...
                    for l_layer in self.l_level:
                        ...
                        if outputs:  # Add refinement from previous segment
                            ...
                            if prev_output_hidden.size(-1) != self.hidden_size:
                                ...
                                prev_output_hidden = self._refinement_proj(
                                    prev_output_hidden
                                )
...
class HRMReasoning:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), lr=self.hrm_cfg.get("learning_rate", 1e-4)
        )
    ...
    def _learn_from_experience(self) -> None:
        ...
        for exp in batch:
            if exp["done"]:
                ...
                with torch.no_grad():
                    ...
                    halt_target = (
                        float(exp["reward"]) + 0.99 * max_next_q * 0.8
                    )  # Slight discount for halting
        ...
        q_halt_targets = torch.tensor(
            q_halt_targets, dtype=torch.float, device=self.device
        )
        q_continue_targets = torch.tensor(
            q_continue_targets, dtype=torch.float, device=self.device
        )
        ...
        self.logger.debug(
            f"Enhanced learning: halt_loss={halt_loss.item():.4f}, continue_loss={continue_loss.item():.4f}"
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        base_status = {
            ...
            "pending_plans": [
                p
                for p in self.tactical_layer
                if not any(a["plan_id"] == p["plan_id"] for a in self.operational_layer)
            ],
        }
        ...
        if hasattr(self.model, "use_rmsnorm"):
            base_status["architecture"] = {
                ...
                "refinement_segments": (
                    self.model.refinement_segments
                    if hasattr(self.model, "refinement_segments")
                    else None
                ),
            }
        ...
        if self.replay_buffer:
            avg_reward = sum(exp["reward"] for exp in self.replay_buffer[-10:]) / min(
                10, len(self.replay_buffer)
            )
            completion_rate = sum(
                1 for exp in self.replay_buffer[-20:] if exp["done"]
            ) / min(20, len(self.replay_buffer))
