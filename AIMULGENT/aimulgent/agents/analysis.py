...
class AnalysisAgent(BaseAgent):
    ...
    async def process_task(self, task_type: str, input_data: Dict[str, Any]) -> Any:
        ...
        if task_type == "code_analysis":
            return await self._analyze_code(
                input_data.get("code", ""), input_data.get("file_path")
            )
        else:
    ...
    def _analyze_security(self, code: str) -> List[Dict[str, str]]:
        ...
        for pattern, message in secret_patterns:
            if re.search(pattern, code, re.IGNORECASE):
                issues.append(
                    {"type": "security", "severity": "high", "message": message}
                )
    ...
    def _calculate_quality_score(
        self, metrics: Dict[str, Any], security_issues: List[Dict]
    ) -> float:
    ...
    def _generate_recommendations(
        ...
        if (
            ...
            and metrics.get("lines_of_code", 0) > 10
        ):
